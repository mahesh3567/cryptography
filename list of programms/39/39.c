...
void decryptAdditiveCipher(const char *ciphertext, int shift, char *plaintext) {
    ...
    for (i = 0; i < text_len; ++i) {
        if (isalpha(ciphertext[i])) {
            ...
            char decryptedLetter = ((ciphertext[i] - base - shift + 26) % 26) + base;
...
void additiveCipherLetterFrequencyAttack(const char *ciphertext, int topPlaintexts) {
    ...
    for (i = 0; i < topPlaintexts; ++i) {
        shift = (letterFrequencies[i].letter - 'a' + 26) % 26;
...
int main() {
    ...
    printf("Ciphertext:\n%s\n\n", ciphertext);
    additiveCipherLetterFrequencyAttack(ciphertext, topPlaintexts);
