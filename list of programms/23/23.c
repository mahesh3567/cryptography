...
void encryptSDES(block_t *data, uint16_t key) {
...
void decryptSDES(block_t *data, uint16_t key) {
    block_t ciphertext = *data;
    generateRoundKeys(key, &key, &key);
...
void encryptCTR(block_t *data, uint16_t key, block_t counter) {
    encryptSDES(&counter, key);
    *data ^= counter;
}

void decryptCTR(block_t *data, uint16_t key, block_t counter) {
    encryptSDES(&counter, key);
    *data ^= counter;
}

int main() {
    block_t plaintext = 0b00000001000000100000100;
    block_t expectedCiphertext = 0b001110000100111100110010;
    block_t counter = 0b0000000000000000;
    block_t ciphertext;
    encryptCTR(&plaintext, key, counter);
    ciphertext = plaintext;

    printf("Encrypted Ciphertext: %04X\n", ciphertext);
    printf("Expected Ciphertext:  %04X\n\n", expectedCiphertext);

    decryptCTR(&ciphertext, key, counter);
